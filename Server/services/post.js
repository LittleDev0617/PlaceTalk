...
async function getPosts(options) {    
    ...
    query += ` ORDER BY ${likeOrder ? 'likes' : 'view'} DESC LIMIT ?, ?`;
...
async function isPressLike(post_id, user_id) {
    const { count } = (await conn.query('SELECT COUNT(*) as count FROM tb_likes WHERE post_id = ? AND user_id = ?', [post_id, user_id]))[0];
    return count > 0;
}
...
async function pressPostLike(post_id, user_id) {
    ...
    // 좋아요 누른 상태면
    if(isPressed) {
        await conn.query('DELETE FROM tb_likes WHERE post_id = ?', [post_id]);
        await conn.query('UPDATE tb_post SET likes = ? WHERE post_id = ?', [likes - 1]);
    } else {
        // 안누르면 좋아요 추가
        await conn.query('INSERT INTO tb_likes VALUES(?, ?)', [post_id, user_id]);
        await conn.query('UPDATE tb_post SET likes = ? WHERE post_id = ?', [likes + 1]);
    }
}
module.exports = {
